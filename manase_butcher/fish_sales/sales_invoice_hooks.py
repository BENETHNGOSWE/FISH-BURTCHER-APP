...
def validate_branch_and_kg(doc, method=None):
    ...
    for row in doc.items:
        is_fish = frappe.db.get_value("Item", row.item_code, "custom_mb_is_fish")
        if is_fish:
            if not row.custom_mb_weight_kg:
                row.custom_mb_weight_kg = flt(row.stock_qty or row.qty)
            kg += flt(row.custom_mb_weight_kg)
            warehouse = row.warehouse or doc.set_warehouse
            if warehouse:
                branch = frappe.db.get_value("Warehouse", warehouse, "custom_mb_branch")
                if not doc.custom_mb_branch and branch:
                    doc.custom_mb_branch = branch
    doc.custom_mb_total_kg = flt(kg, 3)
...
def on_cancel_metrics(doc, method=None):
    ...
    log_action("Sales Invoice", doc.name, "Cancel", branch=doc.custom_mb_branch,
